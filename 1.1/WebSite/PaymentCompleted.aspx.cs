        ...
        if (!this.IsPostBack)
        {
            if (transactionNumber != null && transactionNumber != string.Empty)
            {
                ...
                if (processor.GetPDTDetails(transactionNumber, out values, out response))
                {
                    ...
                    string[] customparameter = values["custom"].Split('_');
                    if (customparameter.Length > 1)
                    {
                        string orderNumber = customparameter[0];
                        string username = customparameter[1];
                        PayPalRequest order = null;// OrderManager.GetOrderByID(Convert.ToInt32(orderNumber));

                        //PayPalRequest order = new OrderController().UpdateByOrderID(new Guid(orderNumber), true);
                        if (orderNumber != string.Empty)
                        {
                            ...
                            new OrderController().UpdateByOrderID(new Guid(orderNumber), true);
                            new BusinessLogic.LoginUserController().ActivatUser(username);
                        }
                        Response.Redirect("~/OrderComplete.aspx" + "?st=1");
                    }
                    else
                    {
